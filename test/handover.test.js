...
import { initStore,readState,replayStateFromJournal } from '../src/store.js';
import { ensureProjectKit,REQUIRED_DOCUMENTS,REQUIRED_DECISIONS,projectDocumentPath,readProjectContract,writeProjectContract,projectConfigPath } from '../src/project-kit.js';
...
test('handover blocks incomplete project pack and accepts only completed committed decisions',()=>{
  ...
  sh(root,'git',['add','.gitignore','docs/shipstate','shipstate.project.json']);sh(root,'git',['commit','-m','approve project handover contract']);audit=handoverAudit(root);assert.equal(audit.ready,true);const handover=acceptHandover(root);assert.equal(handover.status,'READY_FOR_HANDOVER');let state=readState(root);assert.ok(state.project.protectedPaths.includes('docs/shipstate/**'));assert.ok(state.project.protectedPaths.includes('shipstate.project.json'));assert.equal(state.delivery.state,'READY_FOR_HANDOVER');state=replayStateFromJournal(root);assert.equal(state.delivery.state,'READY_FOR_HANDOVER');assert.equal(state.team.manager,contract.roles.manager);assert.ok(state.project.protectedPaths.includes('docs/shipstate/**'));
});

test('handover audit is observational and never recreates missing governed files',()=>{const root=tempRepo('handover-readonly');initStore(root);let audit=handoverAudit(root);assert.equal(audit.ready,false);assert.ok(audit.blockers.some(x=>x.type==='config'));assert.equal(fs.existsSync(projectConfigPath(root)),false);ensureProjectKit(root);const required=projectDocumentPath(root,REQUIRED_DOCUMENTS[0]);fs.unlinkSync(required);audit=handoverAudit(root);assert.equal(audit.ready,false);assert.ok(audit.blockers.some(x=>x.type==='document'&&x.key===REQUIRED_DOCUMENTS[0].key));assert.equal(fs.existsSync(required),false);});
