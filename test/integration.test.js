...
test('runner refuses a task with unverified dependencies',()=>{
  const root=temp(); initStore(root); addTasks([task('A'),task('B',['A'])],root);
  assert.throws(()=>runTask('B','dry-run',root),/not eligible/);
  assert.equal(readState(root).tasks.find(t=>t.id==='B').state,'PENDING');
});
