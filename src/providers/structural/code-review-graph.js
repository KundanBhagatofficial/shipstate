...
function exec(root,args,ctx={}){const dir=dataDir(root,ctx);fs.mkdirSync(dir,{recursive:true});const timeout=Math.max(5000,Math.min(Number(ctx.timeoutMs||120000),5*60*1000)),r=runChild('code-review-graph',args,{cwd:root,timeout,maxBuffer:12*1024*1024,extraEnv:{CRG_DATA_DIR:dir},sandboxRoot:root,network:false});if(r.timedOut)throw new Error(`code-review-graph ${args[0]} timed out after ${timeout}ms`);if(r.status!==0)throw new Error((r.stderr||r.stdout||`code-review-graph ${args[0]} failed`).trim().slice(-8000));return {stdout:r.stdout||'',stderr:r.stderr||'',dataDir:dir,sandbox:r.sandbox||null};}
function json(root,args,ctx={}){const r=exec(root,args,ctx);try{return {...JSON.parse(r.stdout||'null'),_shipstate:{dataDir:r.dataDir,sandbox:r.sandbox}};}catch{throw new Error(`code-review-graph returned non-JSON output for ${args[0]}`);}}
