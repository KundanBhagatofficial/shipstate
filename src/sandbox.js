import os from 'node:os';import path from 'node:path';import { run } from './utils.js';
...
function macProfile(worktree,network){return `(version 1)\n(deny default)\n(import \"system.sb\")\n(allow process*)\n(allow file-read*)\n(allow file-write* (subpath \"${worktree.replaceAll('\\','\\\\')}\"))\n${network?'(allow network*)':'(deny network*)'}\n`;}
export function sandboxCommand(spec,{worktree,network=false,memoryMb=0,cpuSeconds=0}){
 ...
 if(d.kind==='bubblewrap'){
   ...
   if(!network)args.push('--unshare-net'); args.push('--',spec.cmd,...spec.args); const pr=run('prlimit',['--version']); if(pr.status===0&&(memoryMb||cpuSeconds)){const limits=[];if(memoryMb)limits.push(`--as=${Math.floor(memoryMb*1024*1024)}`);if(cpuSeconds)limits.push(`--cpu=${Math.floor(cpuSeconds)}`);return {cmd:'prlimit',args:[...limits,'--','bwrap',...args],descriptor:{...d,network,memoryMb,cpuSeconds,resourceLimits:true}};} return {cmd:'bwrap',args,descriptor:{...d,network,memoryMb,cpuSeconds,resourceLimits:false}};
 }
 if(d.kind==='sandbox-exec') return {cmd:'sandbox-exec',args:['-p',macProfile(worktree,network),spec.cmd,...spec.args],descriptor:{...d,network,memoryMb,cpuSeconds,resourceLimits:false}};
 return {...spec,descriptor:{...d,network,memoryMb,cpuSeconds,resourceLimits:false}};
}
