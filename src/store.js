...
export function initStore(root=process.cwd(),name){const p=paths(root);for(const d of [p.base,p.runs,p.contexts,p.worktrees,p.backups,p.cache,p.advisories])ensureDir(d);if(!fs.existsSync(p.state))writeJsonAtomic(p.state,emptyState(root,name));if(!fs.existsSync(p.events))fs.writeFileSync(p.events,'');ensureJournalFormat(root);appendEvent('PROJECT_INITIALIZED',{root,project:readJson(p.state)?.project,profile:readJson(p.state)?.profile,schemaVersion:CURRENT_SCHEMA},root);ensureGitignore(root);return readState(root);}
...
export function verifyJournal(root=process.cwd()){if(!fs.existsSync(paths(root).events))return {valid:true,count:0};let prev='GENESIS',count=0;for(const line of fs.readFileSync(paths(root).events,'utf8').split(/\n/).filter(Boolean)){const e=JSON.parse(line),{hash,...base}=e;if(e.prevHash!==prev||sha256(JSON.stringify(base))!==hash)return {valid:false,count,error:`broken event chain at ${e.id}`};prev=hash;count++;}return {valid:true,count,lastHash:prev};}
...
export function replayStateFromJournal(root=process.cwd()){
  ensureJournalFormat(root);const p=paths(root),lines=fs.readFileSync(p.events,'utf8').split(/\n/).filter(Boolean).map(JSON.parse);let state=null;
  for(const e of lines){
    const q=e.payload||{};
    if(e.type==='PROJECT_INITIALIZED'&&q.project)state={schemaVersion:CURRENT_SCHEMA,project:q.project,profile:q.profile||{},tasks:[],runs:[],evidence:[],decisions:[],metrics:{contexts:[],agentRuns:[],projectTruth:[],providers:[],tokenEconomy:[]},locks:[],plans:[],remotes:[],...autonomousDefaults()};
    if(!state)continue;
    ...
    else if(e.type==='PLAN_CREATED'&&q.plan)state.plans.push(q.plan);
    else if(e.type==='PLAN_APPROVED'){const pl=state.plans.find(x=>x.id===q.planId);if(pl)pl.status='APPROVED';}
    else if(e.type==='METRIC_RECORDED'){state.metrics[q.kind]||=[];state.metrics[q.kind].push(q.item);}
    else if(e.type==='HANDOVER_INITIALIZED')state.handover={...(state.handover||{}),status:'HANDOVER_REVIEW'};
    else if(e.type==='HANDOVER_ACCEPTED')state.handover={...(state.handover||{}),status:'READY_FOR_HANDOVER',contractHash:q.contractHash,roles:q.roles,autonomy:q.autonomy};
    else if(e.type==='PROJECT_HANDED_OVER')state.handover={...(state.handover||{}),status:'HANDED_OVER'};
    else if(e.type==='PROJECT_LIFECYCLE')state.delivery={...(state.delivery||{}),...(q.patch||{}),state:q.state};
    else if(e.type==='OWNER_DECISION_OPENED'&&q.decision)state.ownerDecisions.push(q.decision);
    else if(e.type==='OWNER_DECISION_RESOLVED'){const d=state.ownerDecisions.find(x=>x.id===q.decisionId);if(d){d.status='RESOLVED';d.resolution=q.resolution;d.note=q.note;}}
  ...
  if(!state)throw new Error('Journal does not contain replayable project initialization');writeState(state,root);appendEvent('STATE_REPLAYED',{eventCount:lines.length},root);return state;
}
