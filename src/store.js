...
export function writeState(s,root=process.cwd(),{snapshot=true}={}){const p=paths(root);if(fs.existsSync(p.events))ensureJournalFormat(root);s.schemaVersion=CURRENT_SCHEMA;if(snapshot&&fs.existsSync(p.state)){const current=readValidStateFile(p.state);if(current)writeJsonAtomic(p.prev,current);}writeJsonAtomic(p.state,s);}
