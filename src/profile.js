...
const text=(root,p)=>{try{return fs.readFileSync(path.join(root,p),'utf8');}catch{return '';}};
...
function detectJava(root){
 const gradle=exists(root,'gradlew')||exists(root,'build.gradle')||exists(root,'build.gradle.kts');if(gradle){const gradleText=`${text(root,'build.gradle')}\n${text(root,'build.gradle.kts')}`,android=/com\.android\.(?:application|library)/.test(gradleText)||exists(root,'src/main/AndroidManifest.xml')||exists(root,'app/src/main/AndroidManifest.xml'),hasJava=exists(root,'src/main/java')||exists(root,'app/src/main/java'),hasKotlin=exists(root,'src/main/kotlin')||exists(root,'app/src/main/kotlin'),languages=unique([hasJava?'java':null,hasKotlin?'kotlin':null].filter(Boolean));return {ecosystem:'java',languages:languages.length?languages:['java'],frameworks:android?['android']:[],packageManager:'gradle',commands:{test:'./gradlew test',build:'./gradlew build'}};}
...
function detectSwift(root){ return exists(root,'Package.swift')?{ecosystem:'swift',languages:['swift'],frameworks:['swift-package'],packageManager:'swiftpm',commands:{test:'swift test',build:'swift build'}}:null; }
function detectDart(root){if(!exists(root,'pubspec.yaml'))return null;const p=text(root,'pubspec.yaml'),flutter=/\bsdk\s*:\s*flutter\b/i.test(p)||/^\s*flutter\s*:/m.test(p);return {ecosystem:'dart',languages:['dart'],frameworks:flutter?['flutter']:[],packageManager:flutter?'flutter':'dart',commands:flutter?{test:'flutter test',lint:'flutter analyze'}:{test:'dart test',lint:'dart analyze'}};}
export function detectProject(root=process.cwd()){
  const detectors=[detectNode,detectPython,detectRust,detectGo,detectJava,detectSwift,detectDart].map(f=>f(root)).filter(Boolean);
