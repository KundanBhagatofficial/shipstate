...
export function commandFor(name,ctx){const prompt=`Implement the bounded engineering task using the complete SHIPSTATE execution context at ${ctx}. The manager brief, acceptance criteria, design locks and repair feedback in that context are authoritative. Stay within allowed paths. Do not modify .shipstate, shipstate.project.json, docs/shipstate, .git, or .env files. Do not redefine product scope or architecture. Implement the task completely, then stop; SHIPSTATE owns verification, review and acceptance.`;if(name==='codex')return {cmd:'codex',args:['exec','-s','workspace-write',prompt]};return commandForPrompt(name,prompt);}
