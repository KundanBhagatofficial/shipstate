...
const templates={
  ...
  'TESTING.md':`# Testing and Quality Strategy\n\n${TODO}\n\n## Required Test Layers\nDefine unit, integration, E2E, device/browser and other required tests.\n\n## Platform Matrix\nDefine browser/device/OS coverage required for web, iOS, Android or cross-platform delivery.\n\n## Quality Gates\nDefine lint, typecheck, accessibility, responsive, coverage, performance, security and discoverability thresholds where applicable. Record deterministic commands for each required non-functional dimension in shipstate.project.json qualityCertification.commands.\n\n## Certification\nDefine evidence required before release.\n`,
...
export function defaultProjectContract(){return {
  ...
  qualityCertification:{commands:{}},
  providers:{structuralContext:'auto',semanticContext:'auto',uiIntelligence:'auto',discoverability:'auto'},
  roles:{productOwner:'human',manager:'codex',reviewer:'codex',developer:'claude',developerFallbacks:['codex'],verifier:'shipstate'},
  ...
  limits:{maxRepairCycles:3,maxManagerRetries:2,maxTasksPerSession:100,managerContextTokens:40000,reviewContextTokens:20000,developerContextTokens:12000,developerFailbackProbeMs:300000},
...
export function migrateProjectContract(input={}){const base=defaultProjectContract(),cfg=input||{};return {...base,...cfg,schemaVersion:PROJECT_CONTRACT_SCHEMA,project:{...base.project,...(cfg.project||{}),classes:[...(cfg.project?.classes||[])],platforms:[...(cfg.project?.platforms||[])],publicSurfaces:[...(cfg.project?.publicSurfaces||[])]},qualityProfiles:{...base.qualityProfiles,...(cfg.qualityProfiles||{})},qualityCertification:{...base.qualityCertification,...(cfg.qualityCertification||{}),commands:{...base.qualityCertification.commands,...(cfg.qualityCertification?.commands||{})}},providers:{...base.providers,...(cfg.providers||{})},roles:{...base.roles,...(cfg.roles||{}),developerFallbacks:[...new Set((Array.isArray(cfg.roles?.developerFallbacks)?cfg.roles.developerFallbacks:base.roles.developerFallbacks).map(String).filter(Boolean))]},autonomy:{...base.autonomy,...(cfg.autonomy||{})},limits:{...base.limits,...(cfg.limits||{})},delivery:{...base.delivery,...(cfg.delivery||{})},decisions:{...base.decisions,...(cfg.decisions||{})}};}
