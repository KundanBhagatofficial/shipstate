...
export function cleanupRecordedWorktrees(runs=[],root=process.cwd()){const base=path.resolve(paths(root).worktrees),removed=[];for(const value of new Set((runs||[]).map(r=>r?.worktreePath).filter(Boolean))){const target=path.resolve(value);if(target===base||!target.startsWith(`${base}${path.sep}`))continue;const existed=fs.existsSync(target);removeWorktree(target,root);if(existed)removed.push(target);}return removed;}
...
export function commitChangedFiles(root,commit){const out=git(root,['diff-tree','--no-commit-id','--name-only','-r',commit],{ok:[0]});return out.split('\n').filter(Boolean);}
export function filesChangedBetween(root,from,to='HEAD'){const out=git(root,['diff','--name-only',`${from}..${to}`],{ok:[0]});return out.split('\n').filter(Boolean);}
export function acceptCandidate(candidate,base,root=process.cwd(),{allowDisjointStale=false}={}){if(!isClean(root))throw new Error('Main working tree must be clean before acceptance');const current=head(root);let staleRevalidated=false;if(current!==base){if(!allowDisjointStale)throw new Error('Repository HEAD moved since task execution; refusing stale candidate');if(parentOf(root,candidate)!==base)throw new Error('Stale candidate parent does not match its recorded base');const mainFiles=new Set(filesChangedBetween(root,base,current)),candidateFiles=commitChangedFiles(root,candidate),overlap=candidateFiles.filter(f=>mainFiles.has(f));if(overlap.length)throw new Error(`Stale candidate overlaps intervening main changes: ${overlap.join(', ')}`);staleRevalidated=true;}git(root,['cherry-pick',candidate]);return {integratedCommit:head(root),previousHead:current,staleRevalidated};}
export function abortCherryPick(root=process.cwd()){run('git',['cherry-pick','--abort'],{cwd:root});}
export function resetHard(root,ref){git(root,['reset','--hard',ref]);return head(root);}
